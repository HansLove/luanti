...
core.log("action", "[hashimon_town_desk] loading desk.lua from " .. tostring(core.get_modpath(core.get_current_modname())))
...
function towny.desk.show(name, page)
	...
	if page == "confirm_mayor" then
		...
		core.show_formspec(name, "hashimon_town_desk:panel", table.concat(fs))
		return
	end
...
local function do_found(name, town_name)
	...
	local ok, err = ((towny.desk._orig_town) or core.registered_chatcommands["town"].func)(name, "new " .. town_name)
	if ok then
...
local function do_invite(name, target)
	...
	local ok, err = ((towny.desk._orig_town) or core.registered_chatcommands["town"].func)(name, "invite " .. target)
...
core.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "hashimon_town_desk:panel" then
		return
	end
	...
	if fields.capital then
		...
		if cmd then
			...
			-- Fallback: town spawn
			local ok, err = ((towny.desk._orig_town) or core.registered_chatcommands["town"].func)(name, "spawn")
	...
	if fields.revoke_all then
		local ok, err = ((towny.desk._orig_town) or core.registered_chatcommands["town"].func)(name, "invite revoke all")
	...
	if fields.leave then
		local ok, err = ((towny.desk._orig_town) or core.registered_chatcommands["town"].func)(name, "leave")
	...
	if invs then
		for i, tname in ipairs(invs) do
			if fields["accept_" .. i] then
				local ok, err = ((towny.desk._orig_resident) or core.registered_chatcommands["resident"].func)(
	...
	if roster then
		for i, m in ipairs(roster) do
			if fields["promote_" .. i] then
				local ok, err = ((towny.desk._orig_town) or core.registered_chatcommands["town"].func)(
			...
			if fields["kick_" .. i] then
				local ok, err = ((towny.desk._orig_town) or core.registered_chatcommands["town"].func)(name, "kick " .. m.name)
	...
	core.after(4, function()
		if core.get_player_by_name(name) then
			core.chat_send_player(name,
				"[Pueblo] Espacio+Z o /town abren el Escritorio.")
		end
	end)
end)

core.log("action", "[hashimon_town_desk] desk UI ready.")
